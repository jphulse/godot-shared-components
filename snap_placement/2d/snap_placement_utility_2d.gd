class_name SnapPlacementUtility2D
extends RefCounted

## Default maximum socket distance for considering two 2D sockets aligned.
const DEFAULT_DISTANCE_EPSILON: float = 0.05

## Default maximum facing angle difference for considering two 2D sockets aligned.
...
static func snap_piece_socket_to_target_socket(
		piece: Node2D,
		piece_socket: SnapSocket2D,
		target_socket: SnapSocket2D
) -> void:
	if piece == null or piece_socket == null or target_socket == null:
		return

	var socket_local_transform: Transform2D = piece.global_transform.affine_inverse() * piece_socket.global_transform
	var target_facing_rotation: float = target_socket.global_rotation + PI
	var target_facing_transform: Transform2D = Transform2D(target_facing_rotation, target_socket.global_position)
...
static func snap_and_connect_piece_socket_to_target_socket(
	...
	if piece.has_connected_sockets():
		push_warning("SnapPlacementUtility2D refused to snap a piece that already has connected sockets. Disconnect it first or use a cluster-aware placement utility.")
		return false

	if not piece_socket.can_connect_to(target_socket):
		return false

	snap_piece_socket_to_target_socket(piece, piece_socket, target_socket)
...
static func can_connect_aligned_sockets(
		socket_a: SnapSocket2D,
		socket_b: SnapSocket2D,
...
static func connect_sockets(socket_a: SnapSocket2D, socket_b: SnapSocket2D) -> bool:
	if socket_a == null or socket_b == null:
		return false
...
static func disconnect_sockets(socket_a: SnapSocket2D, socket_b: SnapSocket2D) -> void:
...
static func auto_connect_piece_to_neighbors(
		placed_piece: SnapPiece2D,
		neighbor_pieces: Array[SnapPiece2D],
	...
	for placed_socket: SnapSocket2D in placed_piece.get_auto_connect_open_sockets():
		for neighbor_piece: SnapPiece2D in neighbor_pieces:
			...
			for neighbor_socket: SnapSocket2D in neighbor_piece.get_auto_connect_open_sockets():

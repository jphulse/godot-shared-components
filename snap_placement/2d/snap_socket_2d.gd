class_name SnapSocket2D
extends Marker2D

## Emitted when this socket connects to another socket.
signal socket_connected(self_socket: SnapSocket2D, other_socket: SnapSocket2D)

## Emitted when this socket disconnects from another socket.
signal socket_disconnected(self_socket: SnapSocket2D, other_socket: SnapSocket2D)
...
var connected_socket: SnapSocket2D = null
...
func can_connect_to(other_socket: SnapSocket2D) -> bool:
...
func can_auto_connect_to(other_socket: SnapSocket2D) -> bool:
...
func connect_to_socket(other_socket: SnapSocket2D) -> void:
...
func disconnect_socket() -> void:
	var previous_connection : SnapSocket2D = connected_socket
	connected_socket = null
	is_connected = false
	if previous_connection != null:
		socket_disconnected.emit(self, previous_connection)


## Returns this socket's outward-facing direction in global 2D space.
func get_facing_direction() -> Vector2:
	var facing_direction: Vector2 = Vector2.RIGHT.rotated(global_rotation)
	return facing_direction.normalized()


## Returns whether this socket is facing another socket within the given angle limit.
func is_facing_socket(other_socket: SnapSocket2D, max_angle_degrees: float = 5.0) -> bool:
	if other_socket == null:
		return false

	var this_direction: Vector2 = get_facing_direction()
	var other_direction: Vector2 = other_socket.get_facing_direction()
	var angle_difference: float = abs(this_direction.angle_to(-other_direction))
